...
public class JSEProviderImpl implements JJJVMProvider {
    ...
    private static String extractTypeFromFieldSignature(final String fieldSignature) {
        ...
        for (int i = 0; i < fieldSignature.length(); i++) {
            ...
            if (className) {
                ...
                if (chr == JJJVMConstants.TYPE_CLASS) {
                    className = true;
                }
                buffer.append(chr);
            }
    ...
    protected static Method findMethod(final Class<?> klazz, final String methodName, final Class[] types) {
        Method found = null;
        try {
     ...
     */
    public static Class findClassForNameInHierarchy(final Class klazz, final String normalClassName) {
        if (klazz == null) {
            return null;
        }
            ...
            if (clazzName) {
                ...
                switch (chr) {
                    ...
                    case ')':
                        ...
                        i = signature.length();
                        break;
                    case JJJVMConstants.TYPE_CLASS:
                        klazz = null;
                        clazzName = true;
                        break;
                    case JJJVMConstants.TYPE_INT:
